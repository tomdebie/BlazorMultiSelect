...
export function init(multiSelect, dotNetObjRef) {
    const eventHandlers = [];
    ...
    // Helper to track event listeners for dispose
    function addListener(target, type, handler, options) {
        target.addEventListener(type, handler, options);
        eventHandlers.push({ target, type, handler, options });
    }

    function removeAllListeners() {
        eventHandlers.forEach(({ target, type, handler, options }) => {
    ...
    function toggleDropdown(open) {
        ...
        if (isOpen) {
            ...
            addListener(document, 'click', autoCloseDropdown, true)
        } else {
            currentFocusIndex = -1;
            selectButton.focus();
            eventHandlers
                .filter(h => h.target === document && h.type === 'click')
                .forEach(h => document.removeEventListener('click', h.handler, h.options));
        }
    }

    addListener(selectButton, 'click', () => toggleDropdown());
    ...
    addListener(dropdown, 'click', (e) => e.stopPropagation());

    // Keyboard navigation for button
    addListener(selectButton, 'keydown', (e) => {
        switch (e.key) {
            case 'Enter':
            case ' ':
            case 'ArrowRight':
    ...
    addListener(dropdown, 'keydown', (e) => {
    ...
    elementListeners.set(multiSelect, { removeAllListeners });
}

export function dispose(element) {
    const record = elementListeners.get(element);
    if (!record) return;
